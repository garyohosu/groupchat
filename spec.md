...
- メッセージを古い順で表示
- 初回表示は直近 50 件（`before`/`after` なし）
- 過去ログ読み込みボタンでさらに取得可能（`before=<messageId>` を使用）
- 投稿者名、本文、時刻を表示
- 自分の投稿のみ削除ボタンを表示
- ポーリング（3〜5秒間隔）で新着を取得（`after=<messageId>` を使用）
...
| after | 任意 | このIDより大きいメッセージを取得（新着ポーリング用） |

- 最大50件を返す
- 初回表示は `before`/`after` なし
...
- `hasMore`: 過去ログ取得時にさらに古いメッセージが存在するかどうか（`after` 利用時は省略可）
